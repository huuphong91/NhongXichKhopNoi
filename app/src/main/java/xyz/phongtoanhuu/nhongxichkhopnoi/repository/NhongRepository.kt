...
import xyz.phongtoanhuu.nhongxichkhopnoi.utils.SPROCKET_TYPE

class NhongRepository(
    ...
    fun getSprocketCategories(): LiveData<Resource<List<CategoryEntity>>> {
        return object :
            ...
            override fun loadFromDb(): LiveData<List<CategoryEntity>> {
                return appDatabase.categoryDao().getCategories(SPROCKET_TYPE)
            }

            override fun createCall(): LiveData<ApiResponse<List<CategoryResponse>>> {
                return serverService.getSprocketCategories()
            }
        }.asLiveData()
    }
}